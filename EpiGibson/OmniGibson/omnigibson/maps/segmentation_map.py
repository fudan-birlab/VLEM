...
class SegmentationMap(BaseMap):
    ...
    def get_random_point_by_room_type(self, room_type, mask=None):
        """
        ...
        if mask is None:
            valid_idx = th.where(self.room_sem_map == sem_id)
        else:
            valid_idx = th.where((self.room_sem_map * mask) == sem_id)
        valid_idx = th.cat([valid_idx[0][None], valid_idx[1][None]], 0)
    ...
    def get_room_type_by_point(self, xy):
        ...
        if sem_id == 0:
            ...
            for item in self.room_sem_id_to_sem_name.keys():
                if sem_id == item:
                    sem_id = item
                    break
